...
function randomNumber(message){
    let x;
    x = prompt("Press Enter to Randomly Select a " + message);
    let result = Math.floor(Math.random() * 6);  
    return result;
}


function selectAnOption(anArray,message){
    let result;
    let ranNum;
    ranNum = randomNumber(message)
    result = anArray[ranNum]
    return result;    
}


function pickDestination(anArray,message){
   let userInput = "No"
    console.log("Lets get ready to plan the best trip ever, I hope you are ready!");
    console.log(`Lets figure out ${destination}` );    
    while (userInput === "No"){
        destination=selectAnOption(anArray,message);
...
function pickTransportation(anArray,message){
    let userInput = "No"
     console.log(`${destination} is pretty far!`);
     console.log(`Lets figure out ${transportation}`);    
     while (userInput === "No"){
         transportation=selectAnOption(anArray,message)
 ...
 function pickEntertainment(anArray,message){
    let userInput = "No"
     // console.log(``)
     console.log(`${entertainment}`);    
     while (userInput === "No"){
         entertainment=selectAnOption(anArray,message)
